...
def plot_spectrum(results_task):
    ...
    for i_line, data_key_ in enumerate(results_task.items()):
        ...
        for i_step in range(output_data.shape[0]):
            ...
            step_spectrum_diff.append(np.abs(np.fft.fft(output_data[i_step, 2])) -      # 0 for ml, 1 for ap, 2 for v
                                      np.abs(np.fft.fft(output_data[i_step, 2+output_num])))

            # step_spectrum_diff.append(np.abs(np.fft.fft(output_data[i_step, :output_num].ravel())) -      # 0 for ml, 1 for ap, 2 for v
            #                           np.abs(np.fft.fft(output_data[i_step, output_num:].ravel())))
...
if __name__ == "__main__":
    ...
    test_folder = '2023_12_12_23_02_combined_da_ratio'
    ...
    for i_da, (da_name, _) in enumerate(zip(da_names, vgrf_names)):
